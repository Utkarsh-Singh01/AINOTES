          ...
          <AnimatePresence>
            {showCredits && (
              <motion.div
                ...
                className="absolute right-[-8px] mt-4 w-64 rounded-2xl bg-black/90 backdrop-blur-xl border border-white/10 shadow-[0_25px_60px_rgba(0,0,0,0.7)] p-4 text-white"
              >
          ...
          <AnimatePresence>
            {showProfile && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10, scale: 0.95 }}
                transition={{ duration: 0.2 }}
                className="absolute right-0 mt-4 w-53 rounded-2xl bg-black/90 backdrop-blur-xl border border-white/10 shadow-[0_25px_60px_rgba(0,0,0,0.7)] p-4 text-white"
              >
                <MenuItem
                  text="History"
                  onClick={() => {
...
function MenuItem({ onClick, text, red }) {
  return (
    <div
      onClick={onClick}
      className={`w-full text-left px-5 py-3 text-sm transition-colors rounded-lg ${red ? "text-red-400 hover:bg-red-500/10" : "text-gray-200 hover:bg-white/10 cursor-pointer"}`}
    >
      {text}
    </div>
  );
